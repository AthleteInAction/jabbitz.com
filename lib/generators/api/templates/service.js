App.service('<%= name.capitalize %>SVC',['ApiModelV<%= version %>','$timeout',function(ApiModelV<%= version %>,$timeout){
			...
			api_module.save = function(complete){
				...
				if (ID){
					...
					Item.$create(options,function(data){
						...
						api_module.id = data.<%= name %>.id;
						api_module.created_at = data.<%= name %>.created_at;
						api_module.updated_at = data.<%= name %>.updated_at;
